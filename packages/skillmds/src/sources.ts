...
export function parseSource(arg: string, opts: ParseOptions = {}): SourceSpec {
  ...
  // 4. GitHub URLs
  const gh = a.match(/^https?:\/\/(?:www\.)?github\.com\/([^/]+)\/([^/#?]+?)(?:\.git)?(?:\/(tree|blob)\/([^/]+)(?:\/(.*?))?)?\/?(?:[#?].*)?$/);
  if (gh) {
  ...
  const [, owner, repo, subRaw, ref, skill] = m;
  ...
  return withRef({ kind: "github" as const, owner: owner!, repo: repo!.replace(/\.git$/, ""), ref: ref || undefined, subpath, skill: skill || undefined, display: a }, opts.ref);
}
